...
function printMinimalCheatSheet(ctx) {
...
function printCommunityPacks(ctx) {
...
function applyProfile(ctx, profile) {
    ...
    if (profile.systemPrompt)
        ctx.config.systemPrompt = profile.systemPrompt;
    if (profile.maxSteps !== undefined)
        ctx.config.maxSteps = profile.maxSteps;
    if (profile.maxCost !== undefined)
        ctx.config.maxCost = profile.maxCost;
}
function persistCommunityPack(key, profile) {

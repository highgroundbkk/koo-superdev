...
export class TuiRenderer {
    constructor(opts) {
        ...
        this.minimalBatch = new Map();
        this.defaultFormatter = (_name, args) => {
    ...
    renderToolCall(name, callId, args) {
        if (this.display.toolDisplay === 'hidden')
            return;
        this.endStreaming();
    ...
    renderToolResult(name, callId, output) {
        if (this.display.toolDisplay === 'hidden')
            return;
    ...
    flushGrouped() {
        if (this.groupedPending.length === 0)
            return;

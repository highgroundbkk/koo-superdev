...
export function loadConfig(overrides: Partial<AgentConfig> = {}, opts?: { skipApiKey?: boolean }): AgentConfig {
  ...
  if (config.profiles === undefined) config.profiles = DEFAULTS.profiles;
  if (config.loopMode === undefined) config.loopMode = DEFAULTS.loopMode;
  if (config.autoCompact === undefined) config.autoCompact = DEFAULTS.autoCompact;

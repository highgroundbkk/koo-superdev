...
function rainbow(text: string, bold = false): string {
...
function gradient(text: string, from: [number, number, number], to: [number, number, number], bold = false): string {
  let out = '';
  for (let i = 0; i < text.length; i++) {
...
function box(inner: string, border: string): string {
  const pad = Math.max(0, W - vLen(inner));
  return border + '│' + R + inner + ' '.repeat(pad) + border + '│' + R;
}

function frameTop(border: string): string {
  return border + '╭' + '─'.repeat(W) + '╮' + R;
}

function frameMid(border: string): string {
  return border + '├' + '─'.repeat(W) + '┤' + R;
}

function frameBot(border: string): string {
  return border + '╰' + '─'.repeat(W) + '╯' + R;
}

function pixelLine(offset: number, width = W): string {
  const glyphs = ['·', '•', '◦', '▪', '▫'];
...
export async function printBanner(model: string): Promise<void> {
